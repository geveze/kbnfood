  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        // Kategori adından numaraları ve fazla boşlukları kaldır
        categoryName = categoryName.trim().replace(/^\d+\.\s*/, '').trim();
        categoryName = categoryName.replace(/\s+/g, ' '); // Fazla boşlukları kaldır
    ...
    const [categoryCounts] = await conn.execute(
      'SELECT c.name, COUNT(q.id) as count FROM field_inspection_categories c LEFT JOIN field_inspection_questions q ON c.id = q.category_id GROUP BY c.id, c.name'
    );
