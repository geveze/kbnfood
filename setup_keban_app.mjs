...
async function setup() {
  let sysConn, appConn;
  try {
    console.log('📡 TiDB Cloud sys database\'e bağlanıyor...');
    ...
    console.log('✓ sys database\'e bağlantı başarılı');
    
    console.log('\n📦 keban_app database oluşturuluyor...');
    await sysConn.execute('CREATE DATABASE IF NOT EXISTS keban_app');
    ...
    for (const q of questions) {
      const [catResult] = await appConn.execute(
    ...
    const [[{ total: totalQuestions }]] = await appConn.execute('SELECT COUNT(*) as total FROM field_inspection_questions');
    const [[{ total: totalCategories }]] = await appConn.execute('SELECT COUNT(*) as total FROM field_inspection_categories');
    const [[{ total: criticalQuestions }]] = await appConn.execute('SELECT COUNT(*) as total FROM field_inspection_questions WHERE isCritical = 1');
    ...
    console.log(`\n📝 DATABASE_URL: mysql://2UkMMcfEvYMQNtS.root:pRancyW9vAymmp8c@gateway01.eu-central-1.prod.aws.tidbcloud.com:4000/keban_app`);
