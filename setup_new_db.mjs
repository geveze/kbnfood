...
async function setupDatabase() {
  let connection;
  try {
    console.log('📡 TiDB Cloud\'a bağlanıyor (sys database)...');
    connection = await mysql.createConnection(dbConfig);
    console.log('✓ Bağlantı başarılı!');
    
    // keban_app database oluştur
    console.log('\n📦 keban_app database oluşturuluyor...');
    await connection.execute('CREATE DATABASE IF NOT EXISTS keban_app');
    console.log('✓ keban_app database oluşturuldu');
    
    // keban_app database'e bağlan
    console.log('\n📡 keban_app database\'e bağlanıyor...');
    ...
    console.log('✓ keban_app database\'e bağlantı başarılı!');
    
    // Test sorgusu
    const [testResult] = await newConnection.execute('SELECT 1 as test');
    console.log('✓ Test sorgusu başarılı');
    
    // Tablolar oluştur
    console.log('\n📋 Tablolar oluşturuluyor...');
    
    // field_inspection_categories
    ...
    for (const q of questions) {
      const [catResult] = await newConnection.execute(
    ...
    // Doğrulama
    console.log('\n✅ Doğrulama yapılıyor...');
    const [[{ total: totalQuestions }]] = await newConnection.execute('SELECT COUNT(*) as total FROM field_inspection_questions');
    const [[{ total: totalCategories }]] = await newConnection.execute('SELECT COUNT(*) as total FROM field_inspection_categories');
    const [[{ total: criticalQuestions }]] = await newConnection.execute('SELECT COUNT(*) as total FROM field_inspection_questions WHERE isCritical = 1');
    ...
    console.log(`\n🔒 Veritabanı: keban_app`);
    console.log(`🌐 Host: gateway01.eu-central-1.prod.aws.tidbcloud.com`);
    console.log(`📝 DATABASE_URL: mysql://2UkMMcfEvYMQNtS.root:pRancyW9vAymmp8c@gateway01.eu-central-1.prod.aws.tidbcloud.com:4000/keban_app`);
