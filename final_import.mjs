...
console.log('\n[Import] Veriler yükleniyor...');
const sql = fs.readFileSync('/tmp/import_correct_data.sql', 'utf-8');
const statements = sql.split(';').filter(s => s.trim() && !s.trim().startsWith('--'));

console.log(`[Import] ${statements.length} statement bulundu`);
...
for (const stmt of statements) {
  try {
    ...
    if (trimmed) {
      ...
      executed++;
      if (executed % 50 === 0) {
        console.log(`[Import] ${executed} statement executed...`);
      }
    }
  } catch (error) {
    errors++;
...
console.log(`\n[Import] Tamamlandı: ${executed} executed, ${errors} errors`);
