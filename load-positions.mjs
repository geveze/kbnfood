...
async function loadPositionsData() {
  ...
  try {
    ...
    console.log('Adding missing columns to positions table...');
    try {
      ...
      console.log('  (displayName column may already exist)');
    }
    ...
    console.log('Modifying questionNumber column type...');
    try {
      await connection.execute(`
        ALTER TABLE \`position_questions\` MODIFY COLUMN \`questionNumber\` int NOT NULL
      `);
      console.log('✓ Modified questionNumber column');
    } catch (e) {
      console.log('  (Column modification may have failed, continuing...)');
    }

    console.log('Clearing existing data...');
    await connection.execute('DELETE FROM position_questions');
    await connection.execute('DELETE FROM position_categories');
    await connection.execute('DELETE FROM positions');
    console.log('✓ Cleared existing data');

    console.log('Inserting positions data...');
    const positionsSQL = `
    ...
    `;
    await connection.execute(positionsSQL);
    console.log('✓ Positions inserted');

    console.log('Inserting position_categories data...');
    const categoriesSQL = `
    ...
    console.log('Inserting position_questions data...');
    const questionsSQL = `
    ...
    `;
    await connection.execute(questionsSQL);
