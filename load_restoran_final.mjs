...
const pos = await db.select().from(positions).where(eq(positions.name, 'RESTORAN_YONETIMI'));
...
const existingCats = await db.select().from(positionCategories).where(eq(positionCategories.positionId, positionId));

for (const cat of existingCats) {
  await db.delete(positionQuestions).where(eq(positionQuestions.categoryId, cat.id));
}

await db.delete(positionCategories).where(eq(positionCategories.positionId, positionId));
...
for (const [categoryName, questions] of Object.entries(categoriesData)) {
  const catInsert = await db.insert(positionCategories).values({
    positionId,
    name: categoryName
  });
  
  // Kategori ID'sini al
  const cats = await db.select().from(positionCategories).where(eq(positionCategories.name, categoryName));
  const categoryId = cats[cats.length - 1].id;
