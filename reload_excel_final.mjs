...
async function loadExcelQuestions() {
  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        categoryName = categoryName.trim()
          .replace(/^\d+\.\s*/, '') // Başındaki numaraları kaldır
        ...
        if (!categoryId) {
          // Hiçbir kategori bulunmazsa atla
          continue;
        }
    ...
    const [categoryCounts] = await conn.execute(
      'SELECT c.name, COUNT(q.id) as count FROM field_inspection_categories c LEFT JOIN field_inspection_questions q ON c.id = q.category_id GROUP BY c.id, c.name ORDER BY c.id'
    );
